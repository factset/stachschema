(function () {
  var install = function(hook, vm) {
    refTargetPattern = /\[([^\]]*)\]\(([a-zA-Z0-9.]+) +:ref( +:(v1|v2))?\)/mg;
      ...
      while (match != null) {
        var replacement = {
          ...
          replace: function(content) {
            ...
            var id = this.type.replace(/\./mg, '-').toLowerCase();
            var version = this.version || 'v2';
            var replacementString = '<a class="reference" href="#/' + version + '/Reference?id=' + id + '" title="' + version + ': ' + this.type + '"><code>' + label + '</code></a>';
            var result = content.substring(0, this.index) + replacementString + content.substring(this.index + this.length);
            return result;
          }
        };
